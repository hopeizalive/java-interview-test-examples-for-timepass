...
/**
 * Chunked parallel processing and parallel-stream caveats (lesson 58).
 */
public final class DemoChunks {
    ...
    public static void l58(StudyContext ctx) throws Exception {
        ...
        int chunkSize = 200;
        // Execution story step 1: partition one dataset into fixed-size ranges.
        try (var pool = Executors.newFixedThreadPool(4)) {
            var futures = new ArrayList<java.util.concurrent.Future<Integer>>();
            for (int i = 0; i < lines.size(); i += chunkSize) {
                int start = i;
                int end = Math.min(i + chunkSize, lines.size());
                // Each task returns local count for its chunk, simulating independent shard processing.
                futures.add(pool.submit(() -> lines.subList(start, end).size()));
            }
            // Execution story step 2: merge partial results in caller thread.
            int total = 0;
            for (var f : futures) {
                total += f.get();
            }
        ...
        // Execution story step 3: compare with parallelStream convenience and call out common-pool sharing.
